...
//	unsigned int i;
	...
	/* If already fsys power domain is on, just return */
	if ((__raw_readl(EXYNOS5433_FSYS_STATUS) & EXYNOS_INT_LOCAL_PWR_EN) == EXYNOS_INT_LOCAL_PWR_EN) {
		pr_err("[%s] FSYS block already turn on\n", __func__);
		return -EINVAL;
	}
#if 0
	/* Clock enable before turn on power */
	for(i = 0; i < ARRAY_SIZE(gateclks_fsys); i++) {
		/* get clock information */
		gateclks_fsys[i].clock = samsung_clk_get_by_reg((unsigned long)gateclks_fsys[i].reg,
									gateclks_fsys[i].bit_offset);
		if (!gateclks_fsys[i].clock) {
			pr_err("[%s]clock get fail with 0x%08x address[%d]\n",
						__func__, (unsigned int)gateclks_fsys[i].reg,
						gateclks_fsys[i].bit_offset);
			ret = -ENODEV;
			goto out_clk_disable;
		}
		/* Enable clock */
		clk_prepare_enable(gateclks_fsys[i].clock);
	}
#else
	...
	tmp = __raw_readl(EXYNOS5430_ENABLE_SCLK_TOP_FSYS);
	tmp |= 0xFF;
	__raw_writel(tmp, EXYNOS5430_ENABLE_SCLK_TOP_FSYS);

#endif
	/* Turn on FSYS_PMU */
	tmp = __raw_readl(EXYNOS5433_FSYS_CONFIGURATION);
	tmp |= EXYNOS_INT_LOCAL_PWR_EN;
	__raw_writel(tmp, EXYNOS5433_FSYS_CONFIGURATION);

	/* Check FSYS STATUS */
	while((__raw_readl(EXYNOS5433_FSYS_STATUS) & EXYNOS_INT_LOCAL_PWR_EN) != EXYNOS_INT_LOCAL_PWR_EN) {
		if (timeout == 0) {
			pr_err("[%s] option : 0x%08x configuration : 0x%08x status : 0x%08x\n",
					__func__, __raw_readl(EXYNOS5433_FSYS_OPTION),
					__raw_readl(EXYNOS5433_FSYS_CONFIGURATION),
					__raw_readl(EXYNOS5433_FSYS_STATUS));
	...
	/* Release PAD retention for MMC */
	__raw_writel(0x10000000, EXYNOS5433_PAD_RETENTION_MMC0_OPTION);
	__raw_writel(0x10000000, EXYNOS5433_PAD_RETENTION_MMC1_OPTION);
	__raw_writel(0x10000000, EXYNOS5433_PAD_RETENTION_MMC2_OPTION);

	/* clock restore for FSYS block CMU */
	s3c_pm_do_restore_core(exynos_pd_fsys_clk_save,
			ARRAY_SIZE(exynos_pd_fsys_clk_save));
//out_clk_disable:
timeout_err:

	return ret;
}
EXPORT_SYMBOL(exynos_fsys_power_on);

int exynos_fsys_power_off(void)
{
//	unsigned int i;
	unsigned int tmp;
	unsigned int timeout = 10000;
	...
	/* If already fsys power domain is off, just return */
	if ((__raw_readl(EXYNOS5433_FSYS_STATUS) & EXYNOS_INT_LOCAL_PWR_EN) == 0) {
		pr_err("[%s] FSYS block already turn off\n", __func__);
		return -EINVAL;
	}

	/* Before turn off FSYS power domain, Save FSYS clocks */
	s3c_pm_do_save(exynos_pd_fsys_clk_save,
			ARRAY_SIZE(exynos_pd_fsys_clk_save));
#if 0
	/* Clock enable before turn off power */
	for(i = 0; i < ARRAY_SIZE(gateclks_fsys); i++) {
		/* get clock information */
		gateclks_fsys[i].clock = samsung_clk_get_by_reg((unsigned long)gateclks_fsys[i].reg,
									gateclks_fsys[i].bit_offset);
	...
	/* Check FSYS STATUS */
	while((__raw_readl(EXYNOS5433_FSYS_STATUS) & EXYNOS_INT_LOCAL_PWR_EN) != 0) {
		if (timeout == 0) {
			pr_err("[%s] option : 0x%08x configuration : 0x%08x status : 0x%08x\n",
					__func__, __raw_readl(EXYNOS5433_FSYS_OPTION),
					__raw_readl(EXYNOS5433_FSYS_CONFIGURATION),
	...
	/* Set parent of clocks for FSYS to OSC */
	__raw_writel(0x0, EXYNOS5430_SRC_SEL_TOP_FSYS0);
	__raw_writel(0x0, EXYNOS5430_SRC_SEL_TOP_FSYS1);
//out_clk_disable:
timeout_err:

	return ret;
}
EXPORT_SYMBOL(exynos_fsys_power_off);
