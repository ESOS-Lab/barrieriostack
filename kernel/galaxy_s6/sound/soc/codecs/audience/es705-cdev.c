			     ...
			     size_t count, loff_t *f_pos)
{
	...
	while (used < count) {
		...
		if (space == 0) {
			dev_dbg(es705->dev,
				"%s(): line too long - exhausted buffer\n",
				__func__);
			err = -EFBIG;
			goto OUT;
		}
		...
		*end = last;
		...
		dev_dbg(es705->dev, "%s(): used = %zu parse_have = %i\n",
...
static int firmware_open(struct inode *inode, struct file *filp)
{
	...
	if (ret) {
		dev_err(es705->dev,
			"%s(): bus specific boot setup failed: %d\n", __func__,
			ret);
		return ret;
	}
			      ...
			      size_t count, loff_t *f_pos)
{
	...
	dev_dbg(es705->dev, "%s(): firmware write count=%zd bytes\n",
		__func__, count);
	...
	while (wr < count) {
		...
		BUG_ON(bs == 0);
		dev_dbg(es705->dev, "%s(): wr=%zd bs=%zd buf+wr=%p\n",
			       ...
			       struct file *filp)
{
	...
	if (es705->streamdev.open) {
		...
		if (es705->streamdev.config) {
			es705->streamdev.config(es705);
	...
	if (IS_ERR_OR_NULL(stream_thread)) {
		dev_err(es705->dev,
			"%s(): can't create es705 streaming thread = %p\n",
	...
	if (inode->i_cdev == &es705->cdev_streaming) {
		...
		if ((es705_cmd(es705, ES705_DATALOGGING_CMD_ENABLE)) ||
		    (es705->last_response != ES705_DATALOGGING_CMD_ENABLE)) {
			dev_dbg(es705->dev,
				"%s(): failed to turn on datalogging: %d\n",
				__func__, err);
			err = -EBUSY;
			goto OPEN_ERR_UNLOCK_API_MUTEX;
		}
...
static int stream_datalog_release(struct es705_priv *es705, struct inode *inode)
{
	...
	if (inode->i_cdev == &es705->cdev_streaming) {
		...
		if ((es705_cmd(es705, ES705_DATALOGGING_CMD_DISABLE)) ||
		    (es705->last_response != ES705_DATALOGGING_CMD_DISABLE)) {
			dev_warn(es705->dev,
...
static int datalogging_open(struct inode *inode, struct file *filp)
{
	...
	if (es705->streamdev.intf != ES705_UART_INTF) {
		dev_err(es705->dev,
			"%s():  streamdev not using UART interface\n",
			__func__);
		return -EPERM;
	}
				    ...
				    int *nbytes_to_read)
{
	...
	/* If no more bit was found
	 * then get the number of bytes yet to be read
	 */
	if (true == rc) {
...
static int streaming_producer(void *ptr)
{
	...
	if (!buf) {
		pr_err("%s(): -ENOMEM, streaming page allocation\n", __func__);
		return -ENOMEM;
	}
	 ...
	 */
	do {
		if (rlen == PAGE_SIZE || (no_more_bit && !nbytes_to_read)) {
		...
		if (!no_more_bit) {
			no_more_bit = is_nomore_databit_found(0,
							      buf + rlen -
							      rlen_last,
							      rlen_last,
							      &nbytes_to_read);
...
int es705_init_cdev(struct es705_priv *es705)
{
	...
	dev_dbg(es705->dev, "%s(): char dev major = %d\n", __func__,
		cdev_major);
	...
	err = create_cdev(es705, &es705->cdev_command, &command_fops,
			  CDEV_COMMAND);
	if (err) {
		dev_err(es705->dev, "%s(): command cdev is not initialized.\n",
			__func__);
		goto err_command;
	}
	...
	err = create_cdev(es705, &es705->cdev_firmware, &firmware_fops,
			  CDEV_FIRMWARE);
	if (err) {
		dev_err(es705->dev, "%s(): firmware cdev is not initialized.\n",
			__func__);
		goto err_firmware;
	}
	...
	if (err) {
		dev_err(es705->dev,
			"%s(): streaming cdev is not initialized.\n", __func__);
		goto err_streaming;
	}
	...
	if (err) {
		dev_err(es705->dev,
			"%s(): datablock cdev is not initialized.\n", __func__);
		goto err_datablock;
	}
	...
	if (err) {
		dev_err(es705->dev,
			"%s(): datalogging cdev is not initialized.\n",
			__func__);
