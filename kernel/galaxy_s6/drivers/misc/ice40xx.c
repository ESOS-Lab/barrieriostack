...
static char *str_states[] = {"on_i2c", "off_i2c"};
enum {
	I_STATE_ON_I2C = 0,
	I_STATE_OFF_I2C,
};
...
{
	int ret;
	static struct regulator *regulator_vccio = NULL;

	if(!regulator_vccio) {
		regulator_vccio = regulator_get(NULL, g_pdata->regulator_vccio);
	}

	if (onoff) {
		ret = regulator_enable(regulator_vccio);
		if (ret) {
			printk(KERN_ERR"enable s2mpb02-ldo18 failed, rc=%d\n", ret);
			return;
		}
		printk(KERN_DEBUG"ir_led power_on is finished.\n");
	} else {
		if (regulator_is_enabled(regulator_vccio)) {
			ret = regulator_disable(regulator_vccio);
			if (ret) {
				printk(KERN_ERR"disable ldo18 failed, rc=%d\n",
						ret);
				return;
			}
		}
			...
			struct irda_ice40_platform_data *pdata)
{
	...
	if (of_property_read_string(np, "irda_ice40,regulator_vccio", &pdata->regulator_vccio)) {
		pr_err("Failed to get regulator_vccio name property\n");
		return -EINVAL;
	}
	...
	pdata->i2c_gpio = of_property_read_bool(np, "irda_ice40,i2c-gpio");
...
static int irda_i2c_pinctrl_init(struct irda_ice40_data *data)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (IS_ERR(data->pin_state[i])) {
			pr_irda("%s: Failed to get pinctrl state\n", __func__);
...
int irda_i2c_pinctrl(struct irda_ice40_data *data, int state)
{
	...
	if (ret < 0) {
		pr_irda("%s: Failed to configure irq pin\n", __func__);
		return ret;
	}
...
static void irda_ice40_config(void)
{
	...
	pr_irda("%s\n", __func__);
	irda_i2c_pinctrl(g_data,I_STATE_OFF_I2C);
	rc = gpio_direction_output(g_pdata->spi_si, 0);
	if (rc)
		pr_err("%s: spi_si error : %d\n", __func__, rc);
	rc = gpio_direction_output(g_pdata->spi_clk, 0);
	if (rc)
	...
	rc = gpio_direction_output(g_pdata->cresetb, 0);	
	if (rc)
		pr_err("%s: rst_n error : %d\n", __func__, rc);
...
static int ice40_fpga_firmware_update_start(const u8 *data, int len)
{
	...
	usleep_range(50, 70);
	fpga_enable(0,0);
	irda_i2c_pinctrl(g_data,I_STATE_ON_I2C);
		...
		const struct i2c_device_id *id)
{
	...
	if (error == 0) {
		pr_irda("%s,No I2C functionality found\n", __func__);
		return -ENODEV;
	}
	...
	irda_i2c_pinctrl_init(data);
	irda_ice40_config();

	irda_ice40_dev = sec_device_create(data, "sec_ir");
...
static struct i2c_driver ice40_i2c_driver = {
	.driver = {
		.name = "irda_ice40",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(irda_ice40_match_table),
	},
...
{
	pr_irda("%s\n", __func__);
	i2c_add_driver(&ice40_i2c_driver);
	return 0;
}
