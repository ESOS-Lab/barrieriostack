	...
	if (driver->logging_mode == MEMORY_DEVICE_MODE) {
		if (data_type == APPS_DATA) {
			...
			if (foundIndex == -1)
				...
				pr_debug("diag: ENQUEUE HSIC buf ptr and length is %p , %d, ch %ld\n",
				...
				if (write_ptr_mdm) {
					...
					if (err) {
						diagmem_free(driver,
							...
							index +
							POOL_TYPE_HSIC_WRITE);
						pr_err("diag: HSIC write failure, err: %d, ch %ld\n",
							err, index);
					}
				} else {
			  ...
			  int type)
{
	...
	if (type == SMD_DATA_TYPE) {
		switch (peripheral) {
		case MODEM_DATA:
			smd_info->wq = create_freezable_workqueue(
...
int diagfwd_init(void)
{
	...
	driver->diag_wq = create_freezable_workqueue("diag_wq");
	if (!driver->diag_wq)
		goto err;
	driver->diag_usb_wq = create_freezable_workqueue("diag_usb_wq");
