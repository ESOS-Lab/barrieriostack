				...
				const struct i2c_device_id *dev_id)
{
	...
	if (s2mpb02_read_reg(s2mpb02->i2c, S2MPB02_REG_BST_CTRL2, &reg_data) < 0) {
		pr_err("%s: device not found on this channel!!\n", __func__);
		ret = -ENODEV;
		goto err;
	} else {
		if(reg_data == 0x90) {
			S2MPB02_PMIC_REV(s2mpb02) = 1;
			s2mpb02_write_reg(s2mpb02->i2c, S2MPB02_REG_BST_CTRL1, 0xAC); /* OTP work-around */
		}
		else
