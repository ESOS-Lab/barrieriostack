...
void lcd_init(struct decon_lcd *lcd)
{
	...
	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_REG_F2,
				ARRAY_SIZE(SEQ_REG_F2)) < 0)
		dsim_err("fail to write F2 init command.\n");

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_REG_F9,
	...
	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_TOUCH_HSYNC,
	...
	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_COLUMN_ADDRESS,
				ARRAY_SIZE(SEQ_COLUMN_ADDRESS)) < 0)
		dsim_err("fail to write COLUMN_ADDRESS init command.\n");

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_GAMMA_CONDITION_SET,
				ARRAY_SIZE(SEQ_GAMMA_CONDITION_SET)) < 0)
		dsim_err("fail to write GAMMA_CONDITION_SET init command.\n");

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_AID_SET,
				ARRAY_SIZE(SEQ_AID_SET)) < 0)
		dsim_err("fail to write AID_SET init command.\n");

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_ELVSS_SET,
				ARRAY_SIZE(SEQ_ELVSS_SET)) < 0)
		dsim_err("fail to write ELVSS_SET init command.\n");

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_GAMMA_UPDATE,
				ARRAY_SIZE(SEQ_GAMMA_UPDATE)) < 0)
		dsim_err("fail to write GAMMA_UPDATE init command.\n");

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_ACL_OFF,
				ARRAY_SIZE(SEQ_ACL_OFF)) < 0)
		dsim_err("fail to write ACL_OFF init command.\n");

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_ACL_OPR,
				ARRAY_SIZE(SEQ_ACL_OPR)) < 0)
		dsim_err("fail to write ACL_OPR init command.\n");

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_HBM_OFF,
				ARRAY_SIZE(SEQ_HBM_OFF)) < 0)
		dsim_err("fail to write HBM_OFF init command.\n");

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_TSET_GLOBAL,
				ARRAY_SIZE(SEQ_TSET_GLOBAL)) < 0)
		dsim_err("fail to write TSET_GLOBAL init command.\n");

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (unsigned long)SEQ_TSET,
...
{
	if (dsim_wr_data(MIPI_DSI_DCS_SHORT_WRITE, (unsigned long)SEQ_DISPLAY_ON[0], 0) < 0)
