/* drivers/video/exynos/decon/panels/s6e3ha0_mipi_lcd.c
 *
...
#if defined(CONFIG_EXYNOS_DECON_MDNIE_LITE)
extern int s6e3ha0_send_seq(struct dsim_device *dsim, struct lcd_seq_info *seq, u32 num);
extern int s6e3ha0_read(struct dsim_device *dsim, u8 addr, u8 *buf, u32 size);
#endif
...
static int s6e3ha0_wqhd_get_brightness(struct backlight_device *bd)
{
	return bd->props.brightness;
}

static int s6e3ha0_wqhd_set_brightness(struct backlight_device *bd)
{
...
static const struct backlight_ops s6e3ha0_wqhd_backlight_ops = {
	.get_brightness = s6e3ha0_wqhd_get_brightness,
	.update_status = s6e3ha0_wqhd_set_brightness,
};
...
static int s6e3ha0_wqhd_probe(struct dsim_device *dsim)
{
	...
	priv->bd = backlight_device_register("panel", dsim->dev, &dsim->priv, &s6e3ha0_wqhd_backlight_ops, NULL);
...
#if defined(CONFIG_EXYNOS_DECON_MDNIE_LITE)
	mdnie_register(&dsim->lcd->dev, dsim, (mdnie_w)s6e3ha0_send_seq, (mdnie_r)s6e3ha0_read);
#endif
...
static int s6e3ha0_wqhd_suspend(struct dsim_device *dsim)
{
	dsim_info("%s was called\n", __func__);
...
static int s6e3ha0_wqhd_resume(struct dsim_device *dsim)
{
	dsim_info("%s was called\n", __func__);
	return 0;
}

struct mipi_dsim_lcd_driver s6e3ha0_wqhd_mipi_lcd_driver = {
	.probe		= s6e3ha0_wqhd_probe,
	.displayon	= s6e3ha0_wqhd_displayon,
	.suspend	= s6e3ha0_wqhd_suspend,
	.resume		= s6e3ha0_wqhd_resume,
};
