			...
			enum decon_idma_type type)
{
	...
	case IDMA_G0:
	case IDMA_G1:
		decon_write_mask(id, WINCHMAP0, WINCHMAP_DMA(type + 1,
				win_idx), WINCHMAP_MASK(win_idx));
		break;
	case IDMA_VG0:
	case IDMA_VG1:
	case IDMA_VGR0:
...
void decon_ext_reg_config_win_channel(u32 id, u32 win_idx,
			enum decon_idma_type type)
{
}
...
void decon_reg_set_mdnie_blank(u32 id, u32 front,
			u32 sync, u32 back, u32 line)
{
}
