...
//		ret = fimc_is_sec_concord_fw_sel(core, dev, device->pdata, companion_fw_name, master_setf_name, mode_setf_name, 0);
		...
		printk(KERN_INFO "%s: A5 state(0x%lx)\n", __func__, debug);
	...
	u32 indexes)
{
	...
	if (frame) {
		...
		for (index = 0; hindex && (index < 32); index++) {
			flag = 1 << index;
			if (hindex & flag) {
				memcpy((ulong *)(dst_base + ((32 + index) * PARAMETER_MAX_SIZE)),
					(ulong *)(src_base + ((32 + index) * PARAMETER_MAX_SIZE)),
					PARAMETER_MAX_SIZE);
				hindex &= ~flag;
			}
		}
...
int fimc_is_itf_stream_on(struct fimc_is_device_ischain *device)
{
	...
	if ((!pm_qos_request_active(&device->user_qos)) &&
		...
		/* setting dvfs table from HAL, default 0 */
		...
		if (scenario_id >= 0) {
			...
			info("[ISC:D:%d] tbl[%d] static scenario(%d)-[%s]\n",
					device->instance,
	...
	struct fimc_is_minfo *minfo)
{
	...
	imemory->offset_shared	= (ulong)&imemory->is_region->shared[0] -
		imemory->kvaddr;
