...
bool muic_check_is_hv_dev(struct max77833_muic_data *muic_data)
{
	bool ret = false;
...
static int max77833_hv_muic_read_reg(struct i2c_client *i2c, u8 reg, u8 *value)
{
	int ret;

	ret = max77833_read_reg(i2c, reg, value);
	if (ret < 0)
...
static int max77833_hv_muic_write_reg(struct i2c_client *i2c, u8 reg, u8 value)
{
	u8 before_val, after_val;
	int ret;

	max77833_read_reg(i2c, reg, &before_val);
	ret = max77833_write_reg(i2c, reg, value);
	max77833_read_reg(i2c, reg, &after_val);
...
int max77833_muic_hv_update_reg(struct i2c_client *i2c,
	const u8 reg, const u8 val, const u8 mask, const bool debug_en)
{
	...
	if (before_val ^ new_val) {
		ret = max77833_hv_muic_write_reg(i2c, reg, new_val);
		if (ret)
...
void max77833_hv_muic_reset_hvcontrol_reg(struct max77833_muic_data *muic_data)
{
	struct i2c_client *i2c = muic_data->i2c;

	max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVCONTROL1, 0x00);
	max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVCONTROL2, 0x00);
}
...
static int max77833_hv_muic_state_maintain(struct max77833_muic_data *muic_data)
{
	int ret = 0;
...
static void max77833_hv_muic_set_afc_after_prepare
					(struct max77833_muic_data *muic_data)
{
	struct i2c_client *i2c = muic_data->i2c;
	u8 value;
	...
	/* Set HVCONTROL2 = 0x02 */
	max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVCONTROL2,
						HVCONTROL2_DP06EN_MASK);

	/* Set HVCONTROL1 - disable DPVD, DPDNVDEN */
	max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVCONTROL1, 0x20);

	/* Set TX DATA */
	value = muic_data->tx_data;

	max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVTXBYTE, value);

	/* Set HVCONTROL2 = 0x1B */
	max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVCONTROL2, MAX77833_MUIC_HVCONTROL2_1B);

}

static void max77833_hv_muic_set_afc_charger_handshaking
			(struct max77833_muic_data *muic_data)
{
	struct i2c_client *i2c = muic_data->i2c;
	u8 hvtxbyte;
	...
	ret = max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVCONTROL2, 0x13);
	if (IS_ERR_VALUE(ret))
		pr_err("%s:%s cannot write hvcontrol2(%d)\n", MUIC_HV_DEV_NAME, __func__, ret);

	max77833_hv_muic_read_reg(i2c, MAX77833_MUIC_REG_HVTXBYTE, &hvtxbyte);

	for(i = 0; i < HVRXBYTE_MAX; i++) {
		max77833_hv_muic_read_reg(i2c, (MAX77833_MUIC_REG_HVRXBYTE1+i), &hvrxbyte[i]);
		if(hvrxbyte[i] == 0)
			break;
	}
	...
	if(hvrxbyte[0] != hvtxbyte) {
		...
		if(selecthvtxbyte != 0)
			max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVTXBYTE, selecthvtxbyte);
	}

	return;
}

static void max77833_hv_muic_afc_control_ping
		(struct max77833_muic_data *muic_data, bool ping_continue)
{
	int ret;
	...
	if (ping_continue)
		ret = max77833_hv_muic_write_reg(muic_data->i2c, MAX77833_MUIC_REG_HVCONTROL2, 0x5B);
	else
		ret = max77833_hv_muic_write_reg(muic_data->i2c, MAX77833_MUIC_REG_HVCONTROL2, 0x03);
...
static void max77833_hv_muic_qc_charger(struct max77833_muic_data *muic_data)
{
	struct i2c_client	*i2c = muic_data->i2c;
	int ret;
	u8 status3;

	ret = max77833_hv_muic_read_reg(i2c, MAX77833_MUIC_REG_STATUS3, &status3);
	if (ret) {
	...
	case HV_SUPPORT_QC_9V:
		ret = max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVCONTROL1, 0x3D);
		break;
	case HV_SUPPORT_QC_12V:
		ret = max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVCONTROL1, 0x35);
		break;
	case HV_SUPPORT_QC_20V:
		ret = max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVCONTROL1, 0x3F);
		break;
	case HV_SUPPORT_QC_5V:
		ret = max77833_hv_muic_write_reg(i2c, MAX77833_MUIC_REG_HVCONTROL1, 0x33);
		break;
	default:
...
static void max77833_hv_muic_after_qc_prepare(struct max77833_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
...
static void max77833_hv_muic_adcmode_switch
		(struct max77833_muic_data *muic_data, bool always_on)
{
	struct i2c_client	*i2c = muic_data->i2c;
	int ret;
	...
	if (always_on) {
		...
		ret = max77833_muic_hv_update_reg(i2c, MAX77833_MUIC_REG_HVCONTROL1,
					(MAX77833_ENABLE_BIT << HVCONTROL1_VBUSADCEN_SHIFT),
		...
		/* non MAXIM */
		ret = max77833_muic_hv_update_reg(i2c, MAX77833_MUIC_REG_HVCONTROL1,
					(MAX77833_DISABLE_BIT << HVCONTROL1_VBUSADCEN_SHIFT),
					HVCONTROL1_VBUSADCEN_MASK, true);
	}
...
static void max77833_hv_muic_adcmode_always_on(struct max77833_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
	max77833_hv_muic_adcmode_switch(muic_data, true);
}

void max77833_hv_muic_adcmode_oneshot(struct max77833_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
	max77833_hv_muic_adcmode_switch(muic_data, false);
}

static int max77833_hv_muic_handle_attach
		(struct max77833_muic_data *muic_data, const muic_afc_data_t *new_afc_data)
{
	int ret = 0;
	bool noti = true;
	...
	case FUNC_TA_TO_PREPARE:
		max77833_hv_muic_adcmode_always_on(muic_data);
		max77833_hv_muic_set_afc_after_prepare(muic_data);
		muic_data->afc_count = 0;
		muic_data->is_afc_handshaking = false;
		break;
	case FUNC_PREPARE_TO_PREPARE_DUPLI:
		muic_data->afc_count++;
		max77833_hv_muic_set_afc_charger_handshaking(muic_data);
		muic_data->is_afc_handshaking = true;
		if (muic_data->afc_count > AFC_CHARGER_WA_PING) {
			max77833_hv_muic_afc_control_ping(muic_data, false);
		} else {
			max77833_hv_muic_afc_control_ping(muic_data, true);
			noti = false;
		}
		break;
	...
	case FUNC_PREPARE_TO_QC_PREPARE:
		...
		max77833_hv_muic_qc_charger(muic_data);
		max77833_hv_muic_after_qc_prepare(muic_data);
		break;
	...
	case FUNC_PREPARE_DUPLI_TO_AFC_9V:
		max77833_hv_muic_afc_control_ping(muic_data, false);
		max77833_hv_muic_adcmode_oneshot(muic_data);
		break;
...
bool muic_check_dev_ta(struct max77833_muic_data *muic_data)
{
...
static void max77833_hv_muic_detect_dev(struct max77833_muic_data *muic_data, int irq)
{
	struct i2c_client *i2c = muic_data->i2c;
	...
	ret = max77833_bulk_read(muic_data->i2c, MAX77833_MUIC_REG_STATUS1, 3, status);
	if (ret) {
	...
	if (intr == MUIC_INTR_ATTACH) {
		...
		ret = max77833_hv_muic_handle_attach(muic_data, tmp_afc_data);
		if (ret)
		...
		ret = max77833_hv_muic_state_maintain(muic_data);
		if (ret)
...
/* TA setting in max77833-muic.c */
void max77833_muic_prepare_afc_charger(struct max77833_muic_data *muic_data)
{
	struct i2c_client *i2c = muic_data->i2c;
	int ret;
	...
	ret = max77833_muic_hv_update_reg(i2c, MAX77833_MUIC_REG_HVCONTROL1,
			(MAX77833_ENABLE_BIT << HVCONTROL1_DPDNVDEN_SHIFT),
...
/* TA setting in max77833-muic.c */
bool max77833_muic_check_change_dev_afc_charger
		(struct max77833_muic_data *muic_data, muic_attached_dev_t new_dev)
{
	bool ret = true;
...
static void max77833_hv_muic_detect_after_charger_init(struct work_struct *work)
{
	struct afc_init_data_s *init_data =
	    container_of(work, struct afc_init_data_s, muic_afc_init_work);
	struct max77833_muic_data *muic_data = init_data->muic_data;
	int ret;
	u8 status3;
	...
	if (muic_data->is_afc_muic_ready) {
		if (muic_data->is_afc_muic_prepare)
			max77833_hv_muic_detect_dev(muic_data, muic_data->irq_vdnmon);
		else
			max77833_hv_muic_detect_dev(muic_data, -1);
	}

	mutex_unlock(&muic_data->muic_mutex);
}

void max77833_hv_muic_charger_init(void)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
...
static void max77833_hv_muic_check_qc_vb(struct work_struct *work)
{
	struct max77833_muic_data *muic_data =
	    container_of(work, struct max77833_muic_data, hv_muic_qc_vb_work.work);
	u8 status3, vbadc;
	...
	max77833_hv_muic_read_reg(muic_data->i2c, MAX77833_MUIC_REG_STATUS3, &status3);
	vbadc = status3 & STATUS3_VBADC_MASK;

	if (vbadc == VBADC_4V_5V || vbadc == VBADC_5V_6V) {
		muic_data->is_qc_vb_settle = true;
		max77833_hv_muic_detect_dev(muic_data, muic_data->irq_vbadc);
	}
...
void max77833_hv_muic_init_detect(struct max77833_muic_data *muic_data)
{
	int ret;
	u8 status3, vdnmon;
...
void max77833_hv_muic_init_check_dpdnvden(struct max77833_muic_data *muic_data)
{
	u8 hvcontrol1;
	int ret;
	...
	ret = max77833_hv_muic_read_reg(muic_data->i2c, MAX77833_MUIC_REG_HVCONTROL1, &hvcontrol1);
	if (ret) {
...
static irqreturn_t max77833_muic_hv_irq(int irq, void *data)
{
	...
	if (muic_data->is_muic_ready == false)
		...
		max77833_hv_muic_detect_dev(muic_data, irq);
...
do {									\
	ret = request_threaded_irq(_irq, NULL, max77833_muic_hv_irq,	\
...
int max77833_afc_muic_irq_init(struct max77833_muic_data *muic_data)
{
	int ret = 0;
	...
	if (muic_data->mfd_pdata && (muic_data->mfd_pdata->irq_base > 0)) {
		...
		muic_data->irq_vdnmon = irq_base + MAX77833_MUIC_IRQ_INT3_VDNMON;
		REQUEST_HV_IRQ(muic_data->irq_vdnmon, muic_data, "muic-vdnmon");
		muic_data->irq_mrxrdy = irq_base + MAX77833_MUIC_IRQ_MRXRDY;
		REQUEST_HV_IRQ(muic_data->irq_mrxrdy, muic_data, "muic-mrxrdy");
		muic_data->irq_mpnack = irq_base + MAX77833_MUIC_IRQ_INT3_MPNACK;
		REQUEST_HV_IRQ(muic_data->irq_mpnack, muic_data, "muic-mpnack");
		muic_data->irq_vbadc = irq_base + MAX77833_MUIC_IRQ_INT3_VBADC;
...
void max77833_hv_muic_free_irqs(struct max77833_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
...
int of_max77833_hv_muic_dt(struct max77833_muic_data *muic_data)
{
...
void max77833_hv_muic_initialize(struct max77833_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
	...
	INIT_WORK(&afc_init_data.muic_afc_init_work, max77833_hv_muic_detect_after_charger_init);

	INIT_DELAYED_WORK(&muic_data->hv_muic_qc_vb_work, max77833_hv_muic_check_qc_vb);
}

void max77833_hv_muic_remove(struct max77833_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
...
void max77833_hv_muic_remove_wo_free_irq(struct max77833_muic_data *muic_data)
{
	pr_info("%s:%s\n", MUIC_HV_DEV_NAME, __func__);
	cancel_work_sync(&afc_init_data.muic_afc_init_work);
	cancel_delayed_work_sync(&muic_data->hv_muic_qc_vb_work);
