...
static inline size_t sipc5_calc_padding_size(unsigned int len)
{
	size_t residue = len & (sizeof(size_t) - 1);
...
static inline unsigned int sipc5_get_total_len(u8 *frm)
{
	...
	pad = sipc5_padding_exist(frm) ? (unsigned int)sipc5_calc_padding_size(len) : 0;
	return len + pad;
}
