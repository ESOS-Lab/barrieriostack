...
static int init_elvss_table(struct lcd_drvdata *lcd, const u8 *elvss_data)
{
	...
	for (acl = 0; acl < ACL_STATUS_MAX; acl++) {
		...
		/* this (elvss_table[acl][1]) is elvss table to support T ¢®A -20¢®E low temperature */
		for (i = 0; i < ELVSS_STATUS_MAX; i++)
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0, j = 0; i < IV_MAX; i++, j += 3) {
		if (i == 0)
			...
			dsim_info("MTP Offset V%3d R:%04d G:%04d B:%04d\n",
...
static DEVICE_ATTR(test_brightness, 0644, test_brightness_show, test_brightness_store);
...
static int s6e3ha0_probe(struct dsim_device *dsim)
{
	...
	lcd->bd = backlight_device_register("pwm-backlight.0", dsim->dev, lcd, &s6e3ha0_backlight_ops, NULL);
	if (IS_ERR(lcd->bd)) {
