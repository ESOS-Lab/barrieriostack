...
{
	struct mhl_dev_context *mdev = mhid->context;
	uint8_t hb0, hb1;
	int ret;
	...
	int ret, uint8_t hb0)
{
	...
	ret = si_mhl_tx_emsc_add_hid_message(mdev, hb0, 0, out_data, 2);
	if (ret < 0)
 ...
 */
static int mhl3_send_ack(struct mhl3_hid_data *mhid, int ret)
{
	struct mhl_dev_context *context = mhid->context;
...
static void mhl3_hid_init_reports(struct hid_device *hid)
{
	...
	list_for_each_entry(report,
		...
		mhl3_hid_init_report(rep, mhid->in_report_buf, mhid->bufsize);
...
{
	struct mhl_dev_context *context = 0;
 ...
 *
 */
static int mhl3_hid_fetch_hid_descriptor(struct mhl3_hid_data *mhid)
{
...
int mhl3_hid_report_desc_parse(struct mhl3_hid_data *mhid)
{
	...
	if (ret) {
		MHL3_HID_DBG_ERR("hid_parse_report failed\n");
		goto err;
	}
...
static void mhl3_hid_add_work(struct work_struct *workdata)
{
	...
	MHL3_HID_DBG_INFO("%s WORK QUEUE function executing\n", __func__);

	ret = mhl3_hid_fetch_hid_descriptor(mhid);
	if (ret < 0)
		goto err;
...
*/
	...
	MHL3_HID_DBG_INFO("WORK QUEUE function SUCCESS\n");
	mhid->hid_work_active = false;
	return;

err:
	MHL3_HID_DBG_ERR("WORK QUEUE function FAIL\n");
	if (mhid->hid) {
	 ...
	 * timeout.
	 */
	MHL3_HID_DBG_INFO("WAITING FOR ISR_LOCK\n");
	if (down_interruptible(&context->isr_lock)) {
		MHL3_HID_DBG_ERR("Could not acquire isr_lock\n");
		return;
	}

	MHL3_HID_DBG_INFO("ACQUIRED ISR_LOCK\n");
	context->mhl_hid[mhid->id] = 0;
	mhid->hid_work_active = false;
	mhl3_hid_free_buffers(mhid);
	kfree(mhid);

	up(&context->isr_lock);
	MHL3_HID_DBG_INFO("FREED ISR_LOCK\n");
	return;
}
...
static int mhl3_hid_add(struct mhl_dev_context *context, int device_id)
{
	...
	MHL3_HID_DBG_INFO("%s Adding device %d\n", __func__, device_id);

	if (device_id >= MAX_HID_MESSAGE_CHANNELS)
		return -EINVAL;

	if (context->mhl_hid[device_id] == 0) {
		mhid = kzalloc(sizeof(struct mhl3_hid_data), GFP_KERNEL);
		if (!mhid)
			return -ENOMEM;

		context->mhl_hid[device_id] = mhid;
		...
		mhid = context->mhl_hid[device_id];
	}
...
int mhl3_hid_remove(struct mhl_dev_context *context, int device_id)
{
	...
	dummy = down_trylock(&mhid->data_wait_lock);
	up(&mhid->data_wait_lock);
	MHL3_HID_DBG_INFO("%s - cancel_work_sync() returned: %d\n", __func__,
		cancel_work_sync((struct work_struct *)&mhid->mhl3_work));

	if (hid) {
		MHL3_HID_DBG_INFO("%s Disconnecting HID device %d\n",
			__func__, device_id);
 ...
 */
static void mhl3_hid_message_processor(struct mhl_dev_context *mdev,
		uint8_t hb0, uint8_t hb1, uint8_t *pmsg, int length)
{
	...
	int	opState, mhl3_device_id, hid_msg_count, header_len, ret;
	int	msg_channel;
	...
	MHL3_HID_DBG_INFO("Received message: msg_id: %02X\n", msg_id);

	mhid = mdev->mhl_hid[mhl3_device_id];
	opState = OP_STATE_IDLE;
	if (mhid == 0) {
		MHL3_HID_DBG_INFO("Message received with mhid == 0\n");
	...
	case OP_STATE_IDLE:
		if (msg_id == MHL3_DSCRPT_UPDATE) {
			ret = mhl3_hid_add(mdev, mhl3_device_id);
			if (ret < 0)
	...
	case OP_STATE_CONNECTED:
		MHL3_HID_DBG_INFO("OP_STATE_CONNECTED\n");
		...
		case MHL3_REPORT:
			if (msg_channel == HID_ACHID_INT) {
				MHL3_HID_DBG_INFO(
					"Sending %s report to HID-core\n",
					mhid->hid->
					report_enum[HID_INPUT_REPORT].numbered ?
					"numbered" : "");
	...
	if (down_trylock(&mhid->data_wait_lock)) {
		...
		MHL3_HID_DBG_WARN("Sending received msg into the ether!!!\n");
	}
				...
				uint8_t *pmsg, int length)
{
	...
	if (fragment_count == 0) {
		...
		mhl3_hid_message_processor(mdev,
