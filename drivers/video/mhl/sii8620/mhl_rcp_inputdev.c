...
struct rcp_keymap_t rcpSupportTable[MHL_NUM_RCP_KEY_CODES] = {
	...
	{1, 1, 0, {KEY_UNKNOWN, 0},0}, /* 0x05 */
	{1, 1, 0, {KEY_UNKNOWN, 0},0}, /* 0x06 */
	{1, 1, 0, {KEY_UNKNOWN, 0},0}, /* 0x07 */
	{1, 1, 0, {KEY_UNKNOWN, 0},0}, /* 0x08 */

	{0, 0, 0, {KEY_MENU, 0}, (MHL_DEV_LD_GUI)}, /* 0x09 */
	{0, 0, 0, {KEY_SETUP, 0}, 0}, /* 0x0A */
	{0, 0, 0, {KEY_UNKNOWN, 0}, 0}, /* 0x0B */
	{0, 0, 0, {KEY_FAVORITES, 0}, 0}, /* 0x0C */
	...
	{0, 0, 0, {KEY_NUMERIC_0, 0},
		...
		MHL_DEV_LD_MEDIA | MHL_DEV_LD_TUNER | MHL_DEV_LD_ALL)},
	/* 0x21 Numeric 1 */
	...
	/* 0x44 Play */
	{0, 0, 0, {KEY_PLAY, 0}, (MHL_DEV_LD_VIDEO | MHL_DEV_LD_AUDIO | MHL_DEV_LD_ALL)},
	/* 0x45 Stop */
	{0, 0, 0, {KEY_STOP, 0},
		(MHL_DEV_LD_VIDEO | MHL_DEV_LD_AUDIO | MHL_DEV_LD_RECORD | MHL_DEV_LD_ALL)},
	/* 0x46 Pause */
	{0, 0, 0, {KEY_PAUSECD, 0},
		(MHL_DEV_LD_VIDEO | MHL_DEV_LD_AUDIO | MHL_DEV_LD_RECORD | MHL_DEV_LD_ALL)},
	/* 0x47 Record */
	...
	/* 0x48 Rewind */
	{0, 1, 0, {KEY_REWIND, 0}, (MHL_DEV_LD_VIDEO | MHL_DEV_LD_AUDIO | MHL_DEV_LD_ALL)},
	/* 0x49 Fast Forward */
	{0, 1, 0, {KEY_FASTFORWARD, 0}, (MHL_DEV_LD_VIDEO | MHL_DEV_LD_AUDIO | MHL_DEV_LD_ALL)},
	/* 0x4A Eject */
	{0, 0, 0, {KEY_EJECTCD, 0}, (MHL_DEV_LD_MEDIA)},
	/* 0x4B Forward */
	{0, 1, 0, {KEY_NEXTSONG, 0},
		(MHL_DEV_LD_VIDEO | MHL_DEV_LD_AUDIO | MHL_DEV_LD_MEDIA | MHL_DEV_LD_ALL)},
	/* 0x4C Backward */
...
#else
static void rcp_key_report(struct mhl_dev_context *dev_context, uint8_t index)
{
	pr_info("sii8620:%s:%d: report rcp key = %d\n",	__func__, __LINE__, index);
	if (dev_context->rcp_input_dev) {
		input_report_key(dev_context->rcp_input_dev, rcp_def_keymap[index], 1);
		input_report_key(dev_context->rcp_input_dev, rcp_def_keymap[index], 0);
		input_sync(dev_context->rcp_input_dev);
	}
	return;
}
	...
	uint8_t current_key, uint8_t prev_key, enum rcp_event_e event)
{
	...
	if (!(current_key >> 7))
		rcp_key_report(dev_context, current_index);
#else
...
#else
	rcp_input_dev->name	= "sii8620_rcp";
#endif
