...
{
	 long index = (long)(diag_read_ptr->context);
	...
	if (!diag_hsic[index].hsic_ch) {
		pr_err("DIAG in %s: hsic_ch == 0, ch %ld\n", __func__, index);
		return 0;
	}
...
int diagfwd_bridge_init(int idx)
{
	int ret;
	long index ;
	...
	if (index == HSIC_DATA_CH) {
		...
		pr_debug("diag: incorrect bridge instance: %ld\n", index);
		return 0;
	}
	...
	diag_bridge[index].wq = create_freezable_workqueue(name);
	...
	if (index == HSIC_DATA_CH) {
		...
		if (IS_ERR(diag_bridge[index].ch)) {
			pr_err("diag: Unable to open USB MDM ch = %ld\n", index);
			goto err;
		} else
...
int diagfwd_bridge_dci_init(int index)
{
	...
	diag_bridge_dci[index].wq = create_freezable_workqueue(name);
