		...
		if (diag_hsic[i].hsic_device_enabled) {
			if (diag_hsic[i].hsic_device_opened) {
				...
				if (reopen) {
					...
					if (err) {
						pr_err("diag: HSIC %ld channel open error: %d\n",
							 i, err);
					} else {
						pr_debug("diag: opened HSIC channel: %ld\n",
							i);
						diag_hsic[i].
...
void diag_read_usb_hsic_work_fn(struct work_struct *work)
{
	...
	long index = (long)bridge_struct->id;
...
{
	int err = 0;
	long index = (long)hsic_map[pdev_id].struct_idx;
	...
	if ((diag_bridge[index].usb_connected &&
		...
		pr_info("diag: opened HSIC bridge, ch = %ld\n", index);
