...
#include "fips-fmp.h"
#include "fips-fmp-ufs.h"

extern int do_fips_fmp_integrity_check(void);
extern struct fips_fmp_ops fips_fmp_ufs_ops;
...
static int aes_init(struct platform_device *pdev, int mode)
{
	int ret = 0;
...
static int aes_set_iv(struct platform_device *pdev, int mode, uint8_t *iv, uint32_t iv_len)
{
	int ret = 0;
	struct device *dev = &pdev->dev;
	const struct fips_fmp_ops *ops;
	...
	ret = ops->set_iv(pdev, mode, iv, iv_len);
	if (ret) {
...
static int aes_set_key(struct platform_device *pdev, int mode, uint8_t *in_key, uint32_t key_len)
{
	int ret = 0;
	struct device *dev = &pdev->dev;
	const struct fips_fmp_ops *ops;
	...
	ret = ops->set_key(pdev, mode, in_key, key_len);
	if (ret) {
...
static int aes_run(struct platform_device *pdev, uint32_t mode, uint8_t *data, uint32_t len)
{
	int ret = 0;
	struct device *dev = &pdev->dev;
	const struct fips_fmp_ops *ops;
	...
	ret = ops->run(pdev, mode, data, len);
	if (ret) {
...
static int aes_exit(struct platform_device *pdev)
{
	int ret = 0;
	struct device *dev = &pdev->dev;
	const struct fips_fmp_ops *ops;
...
static int fmp_aes_run(struct platform_device *pdev, struct cipher_testvec *template, const int idx,
				uint8_t *data, uint32_t len, const int mode)
{
	int ret;
	struct device *dev = &pdev->dev;

	ret = aes_init(pdev, mode);
	if (ret) {
		dev_err(dev, "Fail to init. ret = %d\n", ret);
		goto err_init;
	}

	ret = aes_set_key(pdev, mode, template->key, template->klen);
	if (ret) {
		dev_err(dev, "Fail to set aes key. ret = %d\n", ret);
		goto err_set_key;
	}

	ret = aes_set_iv(pdev, mode, template->iv, 16);
	if (ret) {
		dev_err(dev, "Fail to set aes iv. ret = %d\n", ret);
		goto err_set_iv;
	}

	ret = aes_run(pdev, mode, data, len);
	if (ret) {
		dev_err(dev, "Fail to set aes iv. ret = %d\n", ret);
		goto err_aes_set;
	}
...
static int test_cipher(struct platform_device *pdev, int mode, struct cipher_testvec *template, uint32_t tcount)
{
	int ret;
	uint32_t idx;
	char *inbuf[XBUFSIZE];
	...
	for (idx = 0; idx < tcount; idx++) {
		...
		indata = inbuf[0];
		outdata = outbuf[0];
		memset(indata, 0, UFS_BLOCK_SIZE);
		memcpy(indata, template[idx].input, template[idx].ilen);

		ret = fmp_aes_run(pdev, &template[idx], idx, indata, template[idx].ilen, mode);
		if (ret) {
			dev_err(dev, "Fail to set aes encrypt run. ret = %d\n", ret);
			goto err_aes_run;
		}

		memset(outdata, 0, UFS_BLOCK_SIZE);
		ret = fmp_aes_run(pdev, &template[idx], idx, outdata, template[idx].ilen, BYPASS_MODE);
		if (ret) {
...
static int do_fips_fmp_selftest(struct platform_device *pdev)
{
	int ret;
	...
	ret = test_cipher(pdev, XTS_MODE, xts_cipher.enc.vecs, xts_cipher.enc.count);
	if (ret) {
	...
	ret = test_cipher(pdev, CBC_MODE, cbc_cipher.enc.vecs, cbc_cipher.enc.count);
	if (ret) {
