...
read_start:
	...
	dbg_log_event(ksb, "B KS_READ", 0, 0);
	...
	dbg_log_event(ksb, "A KS_READ", 0, 0);
	...
	while (!list_empty(&ksb->to_ks_list) && space &&
		...
		if (pkt->n_read == pkt->len) {
			 ...
			 * for more data.
			 */
			if (ksb->udev->dev.power.runtime_status == RPM_ACTIVE) {
				pkt->n_read = 0;
				pkt->len = MAX_DATA_PKT_SIZE;
				submit_one_urb(ksb, GFP_KERNEL, pkt);
			} else
				pr_info("%s: runtime_status: %d, break\n", __func__,
						ksb->udev->dev.power.runtime_status);
			pkt = NULL;
		}
		spin_lock_irqsave(&ksb->lock, flags);
	}
	...
	if (!strcmp(ksb->id_info.name, "efs_hsic_bridge"))
		dev_err(ksb->device, "%s, count:%ld space:%ld copied:%ld",
				 ...
				 size_t count, loff_t *pos)
{
	...
	if (!strcmp(ksb->id_info.name, "efs_hsic_bridge"))
		dev_err(ksb->device, "%s ,count:%ld cmd:%d\n",
				__func__, count, *buf);
...
add_to_list:
	...
	if (wakeup) {
		...
		dbg_log_event(ksb, "W RX_URB", urb->status, urb->actual_length);
	}
done:
	atomic_dec(&ksb->rx_pending_cnt);
	wake_up(&ksb->pending_urb_wait);
...
ksb_usb_probe(struct usb_interface *ifc, const struct usb_device_id *id)
{
	...
	if (device_can_wakeup(&ksb->udev->dev)) {
		...
		device_wakeup_enable(&ksb->udev->dev);
#endif
		ifc->needs_remote_wakeup = 1;
		usb_enable_autosuspend(ksb->udev);
	}

	if (id->idProduct == 0x9008) {
		pm_runtime_forbid(ksb->udev->dev.parent);	/* usb 1 */
		pm_runtime_forbid(&ksb->udev->dev);		/* usb 1-2 */
	}
	ksb->resumed = true;
...
static int ksb_usb_suspend(struct usb_interface *ifc, pm_message_t message)
{
	...
	if (pm_runtime_autosuspend_expiration(&ksb->udev->dev)) {
		...
		pr_info("ksb: SUSP ABORT-TimeCheck");
		return -EBUSY;
	}
...
static int ksb_usb_resume(struct usb_interface *ifc)
{
	...
	if (ksb->resumed) {
		dev_info(&ifc->dev, "%s was already called", __func__);
		return 0;
	}
	ksb->resumed = true;
...
static int __init ksb_init(void)
{
	...
	for (i = 0; i < NO_BRIDGE_INSTANCES; i++) {
		...
		ksb->wq = create_freezable_workqueue(ksb->name);
		if (!ksb->wq) {
