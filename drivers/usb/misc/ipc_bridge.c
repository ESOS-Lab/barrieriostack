...
static int ipc_bridge_submit_inturb(struct ipc_bridge *dev, gfp_t gfp_flags, bool resuming)
{
	int ret;
	unsigned long flags;

	if (atomic_read(&dev->inturb->use_count)) {
		dev_err(&dev->intf->dev, "%s: use_count: %d\n",
				__func__, atomic_read(&dev->inturb->use_count));
		return 0;
	}

	if (!resuming) {
		if (dev->inturb->dev->dev.power.runtime_status == RPM_SUSPENDING ||
			...
			dev_err(&dev->intf->dev, "%s: runtime_status: %d, rx_state %d, state %d\n",
					...
					dev->rx_state, dev->inturb->dev->state);
			return 0;
		}
	}
...
static void ipc_bridge_read_cb(struct urb *urb)
{
	...
	switch (urb->status) {
	case 0:
		DUMP_BUFFER("IPCB Read: ", urb->actual_length, urb->transfer_buffer);
		break;
	...
	if (resubmit) {
		ipc_bridge_submit_inturb(dev, GFP_ATOMIC, false);
	} else {
		spin_lock_irqsave(&dev->lock, flags);
		dev->rx_state = RX_IDLE;
		spin_unlock_irqrestore(&dev->lock, flags);
...
ipc_bridge_write(struct platform_device *pdev, char *buf, unsigned int count)
{
	...
	DUMP_BUFFER("IPCB Write: ", count, buf);
	...
	if(wait_ret == 0){
		...
		dev_err(&dev->intf->dev, "write urb wait timeout %d\n", wait_ret);
		ret = -EBUSY;
		goto done;
	}
...
static int ipc_bridge_resume(struct usb_interface *intf)
{
	...
	return ipc_bridge_submit_inturb(dev, GFP_KERNEL, true);
}
...
ipc_bridge_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	ret = ipc_bridge_submit_inturb(dev, GFP_KERNEL, false);
	if (ret) {
...
#if defined(CONFIG_MDM_HSIC_PM)
	ret = register_udev_to_pm_dev(rmnet_pm_dev, udev);
	if (ret) {
		dev_err(&udev->dev,
...
#if defined(CONFIG_MDM_HSIC_PM)
	unregister_udev_from_pm_dev(rmnet_pm_dev, dev->udev);
#endif
	mutex_destroy(&dev->open_mutex);
	mutex_destroy(&dev->read_mutex);
	mutex_destroy(&dev->write_mutex);
